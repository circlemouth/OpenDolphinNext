...
- **正本ドキュメント**:
  ...
  - web UI デザインの絶対ルールは `docs/web-client/ux/dads-ui-design-absolute-rules.md`。
...
- **UI デザイン作業の絶対条件**:
  - `web-client/` の新規 UI と既存 UI 改修は、必ず `docs/web-client/ux/dads-ui-design-absolute-rules.md` を起点に判断すること。
  - 既存画面や過去メモが上記文書と矛盾する場合は、既存実装ではなく current policy を優先すること。
  - DADS 側で詳細未整備の部品は、安易に独自共通ルールへ昇格させず、必要な場合は個別仕様として理由を残すこと。
