...
const recordApiCall = async (context, { method, pathName, body, query }) => {
  ...
  try {
    ...
    return {
      ...
      ok: response.ok(),
      body: parsed.body,
      parsedBody: parsed.ok,
      responseBodyChars: bodyText.length,
      parsedBodyOk: parsed.ok,
      errorCategory: '',
    };
  } catch (error) {
    networkRecords.push({
  ...
  rejectionReasons
    .map((reason) => {
      ...
      if (reason && typeof reason === 'object') {
        return `${reason.dimension ?? 'unknown'}:${reason.reason ?? 'not_ready'}`;
      }
      return '';
    })
    .filter(Boolean)
  ...
  summary.candidates
    .map(
      (candidate) =>
        ...
        `primary=${candidate.primaryRejectionReason ?? candidate.rejectionReason ?? 'none'}, reasons=${formatRejectionReasons(candidate.rejectionReasons)}, ` +
