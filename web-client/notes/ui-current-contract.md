...
- 共通 `PatientIdentityBar` の医療安全患者ヘッダーは、患者識別情報に加えて、生年月日、性別、年齢、内部参照ID、受付日、診療科、担当医、保険組合せ、ORCA取得 source/fetched/cache status を同じ visible band に表示できる共通 component です。Charts / Reception / Patients はこの共通 contract を使い、ORCA取得が未確認・暫定参照の場合は `stale` / `unverified` として見せ、警告を details 内へ隠しません。
- 共通 `CriticalOperationConfirmDialog` は重大操作の確認 surface として、実行操作名、患者番号、氏名、生年月日、性別、年齢、受付日、診療科、担当医、保険組合せ、ORCA受付ID、対象サマリ、cancel/confirm CTA を同一 modal 内に再掲します。Charts の低レベル ORCA 送信確認では title / operation を `診療行為ORCA送信`、confirm CTA を `ORCAへ送信する` とし、`DiagnosisEditPanel` の病名 ORCA 送信確認では operation を `病名ORCA送信`、title を各操作の確認、confirm CTA を `ORCAへ病名登録` / `ORCA病名を更新` / `ORCA病名を削除` / `削除病名を整理` とします。Charts の通常導線 `診察終了して会計へ送信` は未保存 guard と不足条件 guard を通過した後、共通 alertdialog で患者識別情報、終了対象サマリ、`会計済み確定ではありません` を再掲してから after-finish flow へ進みます。Charts の `その他 > キャンセル` は `診療録取消` として共通 alertdialog を通過し、患者識別情報、取消対象サマリ、`診療録取消の確定ではありません` を再掲してから UI の取消 intent を処理します。診療録確定、診療行為ORCA送信、会計済み確認、診療録取消確定の label と混同させません。共通 modal の action buttons は cancel を secondary、confirm を primary/danger として class contract を分け、DADS の 44px 以上の touch target を CSS と focused test で固定します。
- Charts の `署名確定解除` は共通 `CriticalOperationConfirmDialog` を使い、患者番号、氏名、生年月日、性別、年齢、受付日、診療科、担当医、保険組合せ、ORCA受付ID、予約ID、署名状態、解除段階、影響範囲、`診療録確定や会計済み確定ではありません` を alertdialog 内に再掲します。既存の二段階確認は維持し、解除 callback は最終確認後だけ呼びます。署名確定解除の権限、履歴、再署名要否、監査 enforcement は server-side chart workflow の責務として残します。
- Charts の患者ヘッダーは、患者識別、性別/年齢/生年月日、診療日、状態、保険、診療科/担当医を同じ visible band に整理して表示します。通常状態を説明するだけの `患者安全` バナーは表示せず、未保存、署名確定、ORCA失敗/UNKNOWN/停滞など対応が必要な状態だけ `ChartSafetyBanner` で初期表示します。並行編集ロックは重複した安全バナーへ出さず、常時到達可能な `編集ロック` 警告ボタンと詳細 alertdialog に集約します。これらの警告は患者ヘッダーと同時に見える補助表示であり、診療録確定、ORCA送信、会計済み確定とは混同させません。
- `DiagnosisEditPanel` は ORCA 病名を `ORCA登録病名` panel として表示し、見出し内に `ORCA再取得結果を正本として表示` と件数を compact badge で示します。活動中一覧は `病名 / 属性 / 構成コード / 開始 / 転帰 / 操作` の情報を省略せず、左列ではカード状に圧縮しても病名、コード、開始日、転帰、編集/削除導線を初期表示します。病名一覧の行アクションは画像アイコンボタンで表示し、`編集` / `削除` / `ORCAへ登録` の accessible name と title を保持します。`削除病名を整理` と `ORCAへ病名登録` は引き続き共通重大操作 confirm または入力後 confirm を通過し、ORCA再取得前に local 候補を ORCA登録病名へ混ぜません。
- `PrescriptionOrderEditorPanel` の `オーダー内容` は処方 draft の唯一の編集 surface とし、RP単位の院内/院外、処方区分、用法、日数/回数、請求コメント、備考と、薬剤単位の薬剤名、数量、単位、後発変更可否、一般名/銘柄、患者希望、薬剤コメント、請求コメントを同じ fieldset 内で編集します。薬剤検索と ORCA入力セットは候補を draft へ反映する補助面であり、下部に同じ処方値を再編集する重複フォームを置きません。
...
- `PrescriptionOrderEditorPanel` の `保存` も旧 `POST /api/local/prescription-orders` を使わず、`/api/local/prescription-orders/authority` へ draft mutation を送ります。薬剤 0 件や削除マーカーだけの payload は authority draft を作らず fail-closed で拒否し、旧 local write route を fallback にしません。
...
- Charts の並行編集検知は、患者ヘッダー内の大きな常時ブロックや上部の長文 alert ではなく、常時到達可能な `編集ロック` 警告ボタンと詳細 alertdialog で表示します。通常画面では既存UIを覆わず、read-only 由来の送信不可・会計送信不可・印刷不可の大型 guard note も患者ヘッダー内へ重ねません。詳細 dialog だけに患者ID、受付ID、相手 runId、期限、`最新を再読込`、必要時の `自分の変更を破棄`、`強制引き継ぎ` を出します。`tabLock.isReadOnly` による送信不可・保存不可の enforcement と、強制引き継ぎの二段階確認は維持します。Web 側 tab lock に加えて `/api/local/charts/edit-sessions/*` の TTL lease / heartbeat を使い、他端末の active lease は read-only として扱います。server lease は患者単位の canonical scope で取り、client 提供の受付ID/予約IDで lock scope を分岐しません。ただし lock は一次防壁であり、SOAP/F/Free/order/prescription の保存真正性は server-side content hash / expected revision の 409 で守ります。
...
- 管理画面の `マスタ表示設定` は ORCA master 候補カテゴリの表示/非表示と、施設単位の処方薬剤検索既定値（前方一致/部分一致）だけを管理します。非表示時も手入力、既存入力値、保存済みデータ表示、ORCA送信前の通常検証、`/api/orca/master/order/interactions/check` は維持し、候補未取得・ORCA利用不可・安全確認済みとは表示しません。処方画面はこの既定値を検索 method に使い、患者診療画面内には検索方法の切替 UI を持ちません。
...
- `SoapNotePanel` の `保存` は read-only / 履歴表示 / 保存中では native disabled を維持し、確定済み・履歴表示・保存中などの理由は近傍 `soap-note-save-block-reason` と `aria-describedby` で示します。並行編集ロック由来の read-only は重複文言を出さず、`編集ロック` 警告ボタンと詳細 alertdialog に集約します。
- `SoapNotePanel` の利用者向け見出しは `カルテ本文` とし、内部向けの `Primary Workspace` や折りたたみの記載メタ情報は通常表示に置きません。
- Charts の通常表示では、業務判断に結び付かない説明文を常時表示しません。`当日のオーダーはありません。必要な分野から追加してください。`、`訂正履歴を差分表示します...`、`この列で編集します`、`日付ごとに折りたたみ...` のような空状態/画面構成/操作方法の説明は visible copy から外し、必要なら tooltip、help、details、または screen-reader-only 補足へ退避します。`処方安全チェックサマリ` の正常時説明や `1 RP = 共通用法` の教育的説明も初期表示せず、警告・必須不足・禁忌・相互作用・ORCA UNKNOWN / warning / unmatch がある場合だけ近傍表示します。
- page CTA の owner は `ChartsActionBar` です。通常 UI の primary は `診察終了して会計へ送信` で、`ドラフト保存` / `受付へ戻る` の visible secondary を disclosure 外に置きます。`印刷/エクスポート` はカルテ本文カードの action row で `履歴` の左に置き、実行時は `ChartsActionBar` の印刷ガードと出力 dialog に委譲します。低レベル `ORCA送信` direct bridge は debug / QA / focused test 用に限定し、通常画面の初回会計送信導線には出しません。
- `PastHubPanel` は左列の historical reference / Do 補助 surface であり、comparison 専用主面ではありません。
- Charts の右オーダーペインは当日オーダーの表示・編集主UIです。`OrderDockPanel` はキリンカルテ方式に寄せた `オーダーワークベンチ` として、左列の `処方 / 点滴・注射 / 処置 / 検査 / 算定` 種別、中央の `候補/セット選択`、下段/右側の `オーダー内容` を同一 surface に置きます。カテゴリは件数付きで常時到達可能にし、選択中カテゴリだけに要約または embedded editor を表示します。空カテゴリは大きな説明カードを並べず、右ペイン内で直接入力フォームへ入ります。候補は右オーダーペイン内の検索、頻用、ORCA入力セット、セット/スタンプから下書きへ反映し、候補反映は処方確定・ORCA送信・会計済みではありません。
- `OrderDockPanel` / `OrderBundleEditPanel` / `PrescriptionOrderEditorPanel` の desktop order editor は compact density を標準とし、12px-13px 相当の本文、28px-32px 相当の入力・ボタン高、狭い gap、列見出し中心の表形式で `分類 → 候補 → オーダー内容` を 1 画面内に収めます。患者識別、重要警告、確定/保存/送信確認、local-only 表示は密度削減の対象外です。
- Charts の desktop layout は、左の病名/参照支援列を compact support column とし、浮いた横幅を `charts-order-pane` の当日オーダー確認・編集列へ優先配分します。病名列は ORCA 正本の識別、状態、編集/削除導線を保持しつつ、横幅確保のためにカード状の短縮表示へ寄せます。
- `PrescriptionOrderEditorPanel` は `オーダー内容` RP グリッドを前面に表示し、`RP / 薬剤名称 / 薬剤量 / 成分量 / その他 / 後発変更可否 / 薬剤コメント / 用法 / 日数・回数` を確認・直接編集できる状態にします。RP追加、薬剤行追加、選択薬剤削除、全クリアは同グリッド header の compact action に集約します。薬剤名候補は入力欄のプルダウン候補に統一し、同じ候補を inline table として二重表示しません。成分量とその他は現行データから解決できない場合に `未設定` として表示し、未解決値を成功扱い、確定済み、ORCA送信済みとして扱いません。`処方確定` は引き続き共通重大操作 modal を通し、保存操作から分離します。
- `OrderBundleEditPanel` は処方以外の共通 `オーダー内容` グリッドとして使い、点滴・注射では `薬剤・器材 / 薬剤量・数量 / 成分量 / その他 / コメント / 投与指示 / 速度指定 / 点滴速度` を表示します。`速度指定` と `点滴速度` は `adminMemo` の structured local-only meta として保存し、UI 表示では prefix を露出せず、ORCA送信 payload / XML には投影しません。処置、検査、算定は既存の `entity` と `classCode` allowlist / local-only / import-only / send-block 契約を維持し、グリッド列と候補導線だけを整理します。
- 処方の `RP名`、処方以外の `オーダー名` は通常入力欄として表示しません。保存・候補反映に bundleName が必要な場合は、既存セット名を保持し、未設定なら先頭の薬剤・診療行為・材料名または分類名から client 側で自動補完します。この自動名称は院内ローカル表示/保存補助であり、ORCA正本、処方確定、ORCA送信、会計済み確定の根拠にはしません。
...
- guard minimum:
  ...
  - `SoapNotePanel` save は read-only / 履歴表示 / 保存中で `soap-note-save-block-reason` による近傍理由を表示すること。ただし並行編集ロック由来の read-only は重複表示せず、`編集ロック` 警告ボタンと詳細 alertdialog に集約すること
...
- 業務固有アイコンは文字ラベルの補助として使い、画像アイコンは accessible name を置換しません。`ORCA送信`、`会計送信`、`受付取消`、`診察終了`、`ロック解除` など主要・危険操作は icon-only にしません。表内の省スペース行アクションを画像アイコン化する場合も、実行前確認 modal や最終 CTA では操作名を文字で再掲します。
...
- guard: `verify:medical-safety-ui-copy` は Charts の重大操作 confirm 呼び出しで患者番号、氏名、生年月日、性別、年齢、受付日、診療科、担当医、保険組合せの再掲が欠ける変更を拒否します。主要医療安全画面の `placeholder=` と native `disabled={` は既存残件の上限を cap として固定し、新規追加は labels/support text または `aria-disabled` + 近傍理由へ移すまで拒否します。
...
- 病名マスター候補は補助入力です。`/api/orca/official/disease-master/name/{param}/` は互換 route 名を維持しますが、候補は OpenDolphin local disease candidate cache / projection 由来であり、ORCA 病名正本ではありません。未インポート・取得不能は明示 unavailable として扱い、0 件や bootstrap fallback と混同せず、明示 confirm なしに ORCA 登録 payload や主一覧へ昇格しません。
...
- ORCA 病名操作は `ORCAへ病名登録` / `ORCA病名を更新` / `ORCA病名を削除` / `削除病名を整理` に分け、いずれも共通 `CriticalOperationConfirmDialog` の alertdialogで患者番号、氏名、生年月日、性別、年齢、受付日、診療科、担当医、保険組合せ、ORCA受付ID、操作、病名、属性、ORCA送信コード、再取得待ちを再掲してから `/api/orca/official/chart-support/disease-mod-v3` へ送ります。成功後は楽観更新せず、再取得した ORCA `diseasegetv2` 結果だけを表示します。この確認 modal は患者取り違え防止の UI 補助であり、病名送信の認可、ORCA request number、永続化、監査の server-side enforcement を代替しません。
...
- `MasterVisibilityPanel` は `/api/admin/master-updates/visibility` を使い、固定 6 カテゴリ（処方、注射、処置・手術、検査、病名、患者補助）の UI 候補表示と、処方薬剤検索の施設既定値だけを切り替えます。保存は admin + `admin:mutation` step-up を server-side で必須とし、URL / browser storage / 患者文脈には保存しません。表示文言は master type と影響 UI に限定し、ORCA正本、会計反映、安全確認済みとは表現しません。
