...
- `PrescriptionOrderEditorPanel` は `オーダー内容` RP グリッドを前面に表示し、`RP / 薬剤名称 / 薬剤量 / 成分量 / その他 / 後発変更可否 / 薬剤コメント / 用法 / 日数・回数` を確認できる状態にします。成分量とその他は現行データから解決できない場合に `未設定` として表示し、未解決値を成功扱い、確定済み、ORCA送信済みとして扱いません。`処方確定` は引き続き共通重大操作 modal を通し、保存操作から分離します。
