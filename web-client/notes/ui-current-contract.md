...
- Charts の患者ヘッダーは、患者識別、性別/年齢/生年月日、診療日、状態、保険、診療科/担当医を同じ visible band に整理して表示します。通常状態を説明するだけの `患者安全` バナーは表示せず、未保存、編集ロック、署名確定、ORCA失敗/UNKNOWN/停滞など対応が必要な状態だけ `ChartSafetyBanner` で初期表示します。これらの警告は患者ヘッダーと同時に見える補助表示であり、診療録確定、ORCA送信、会計済み確定とは混同させません。
