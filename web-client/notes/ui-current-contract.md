...
- `診察終了して会計へ送信` の共通重大操作 confirm は患者取り違え防止と誤操作低減の UI 補助です。会計送信可否、encounter close、ORCA transmission、監査、会計済み確定の判断は server-side / owning workflow の enforcement に残し、UI confirm だけで安全性を満たした扱いにしません。
