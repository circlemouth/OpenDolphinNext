...
- `送信候補`: 既存 local-only disease や ORCA 未送信の下書き。`layer=candidate` / `candidateKind=draftCandidate` / `sourceOfTruth=local-candidate` として主病名一覧には混ぜず、対象がある場合だけ隔離枠を表示する。ORCA登録済み病名ではない。
...
- `ORCA未登録の送信候補があります。ORCAへ登録するまで主病名一覧には表示しません。`

## Conflict Matrix
| 状態 | ORCA登録病名 | 診療録本文中の病名記載 | 送信候補 | 候補 | UI / fallback |
| --- | --- | --- | --- | --- | --- |
...
| local-only exists | truth | chart text truth | ORCA未送信候補 | 補助入力 | `送信候補` 枠へ隔離し、主一覧に混ぜない |
...
## Fallback Gates
- UG-04 解決: local候補は正本ではなく `送信候補` 枠に隔離する。
...
- UG-06 解決: local-only と ORCA projection は auto-merge / auto-overwrite せず、local-only は `送信候補` として明示登録まで隔離する。
...
- `KarteDiagnosisService` / `KarteServiceBean` は local病名の create / update / delete service method を公開しません。既存 local-only disease は read-only pending projection としてだけ扱い、正本更新は `diseasev3` operation に限定します。
...
- response は `sourceOfTruth=orca`、`orcaMirrorStatus=connected|unavailable`、主一覧用 `diseases`、隔離表示用 `pendingLocalDiseases` を返す。`pendingLocalDiseases` は `layer=candidate`、`candidateKind=draftCandidate`、`sourceOfTruth=local-candidate`、`candidateOnly=true`、`readOnly=true` を持ち、`diseases` に local-only entry を混ぜません。
...
- 病名送信確認ダイアログは ORCA患者番号、診療日、診療科、保険組合せ、病名属性の表示値、実際に送る ORCA code を同時に表示する。主病名は表示語 `主病名` を payload に入れず、`mainDiseaseClass=01` として送る。疑いは `diseaseSuspectedFlag=S` として表示・送信し、レセプト表示は `diseaseReceiptPrint=1|None`、保険病名は `insuranceDisease=1`、副病名区分は `subDiseaseClass=01..05` として表示値から分離する。追加属性も UI 表示語ではなく `diseaseInsuranceClass=1|0|None`、`diseaseCategory=PD|None`、`diseaseClass=03|04|05|07|08|09|Auto|None`、`diseaseReceiptPrintPeriod=None|00..99`、`dischargeCertificate=0|1|None` の ORCA 仕様コードとして確認画面に表示し、未指定項目は `送信しない` と明示する。
...
- Web client は `needsUserReview=true`、`ORCA_WARNING`、`ORCA_UNMATCHED`、`warnings[]`、`unmatchInformation[]` を成功バナーに潰さず、`ORCA病名送信の要確認` パネルとして初期表示する。表示は固定フィールドの code/category/date などの sanitized summary に限定し、raw XML、内部 URL、資格情報、患者詳細、保険詳細は出さない。
