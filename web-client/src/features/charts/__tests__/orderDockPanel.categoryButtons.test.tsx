...
describe('OrderDockPanel category quick-add', () => {
  it('主要カテゴリ導線を常時表示し quick-add / group-add の data-test-id を保持する', () => {
    ...
    const categoryNav = within(workbench).getByRole('navigation', { name: 'オーダー種別' });
    expect(categoryNav).toBeInTheDocument();
    expect(within(categoryNav).getByRole('button', { name: /処方/ })).toBeInTheDocument();
    expect(within(categoryNav).getByRole('button', { name: /点滴・注射/ })).toBeInTheDocument();
    expect(within(workbench).getByText('候補/セット選択')).toBeInTheDocument();
    expect(within(workbench).getByText('オーダー内容')).toBeInTheDocument();
    expect(within(workbench).getByRole('group', { name: 'オーダー追加' })).toBeInTheDocument();
    expect(within(workbench).getAllByText('全カテゴリ').length).toBeGreaterThan(0);
    expect(within(workbench).getByLabelText('オーダー内容状態')).toHaveTextContent('1件');
    expect(screen.getByText('ORCA候補は確定処方・ORCA送信・会計済みではありません。')).toBeInTheDocument();
    expect(screen.queryByText(/候補ソース:/)).toBeNull();
    expect(screen.queryByText('1 RP = 共通用法。異なる用法は別RP')).toBeNull();
