...
const isSendableMainRow = (row: { source: RpNormalizedRowSource }) =>
  row.source.kind === 'bundle_item' && row.source.rowRole === 'main';
...
export const collectMedicalModV2BundleIssuesForBundle = (bundle: OrderBundle): MedicalModV2BundleIssue[] => {
  ...
  const sendableMainRows = codedRows.filter(isSendableMainRow);
