...
| `surgeryOrder` | 手術 | `500`, `501`, `502`, `510` | sendable | RWO-06I created a safe `500` v1 payload identity and no-live route, but the single live Trial attempt was `businessRejected`; remaining class codes are `no_live_verified` | Do not repeat v1. Identify a sanitized v2 `500` candidate or classify as pending business/Trial data decision; continue to tests only with a separate payload identity. |
| `testOrder` / `laboTest` | 検査 | `600`, `601`, `602`, `603`, `610` | sendable | RWO-06J created a safe `600` v1 payload identity and no-live route, but the single live Trial attempt was `businessRejected`; remaining class codes are `no_live_verified` | Do not repeat v1. Identify a sanitized v2 `600` candidate or classify as pending business/Trial data decision; continue to radiology only with a separate payload identity. |
| `radiologyOrder` | 画像診断 | `700`, `701`, `702`, `703`, `704`, `731`, `732` | sendable | RWO-06K created a safe `700` v1 payload identity and no-live route, but the single live Trial attempt was `businessRejected`; remaining class codes are `no_live_verified` | Do not repeat v1. Identify a sanitized v2 `700` candidate or classify as pending business/Trial data decision; SOAP/disease require separate safe wrappers before any live step. |
...
3. `surgeryOrder` representative `500` 手術 is safely tested but Trial-rejected; do not repeat v1 without a justified v2 candidate.
4. `testOrder` representative `600` 検査 is safely tested but Trial-rejected; do not repeat v1 without a justified v2 candidate.
5. `radiologyOrder` representative `700` 画像診断 is safely tested but Trial-rejected; do not repeat v1 without a justified v2 candidate.
