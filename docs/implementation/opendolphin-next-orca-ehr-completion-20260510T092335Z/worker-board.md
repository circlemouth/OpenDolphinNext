...
| D | `codex/orca-ehr-r2-worker-d-live-orca` | R2-D01 live ORCA Trial execution, adapter contract, retry/idempotency evidence | Active | 20260511T115706Z | this commit | live Trial blocked until same-RUN_ID runtime/candidate/exact preflight/manual approval prerequisites are available |
...
| 20260511T101149Z | D | D-04 follow-up | 10.3/10.4 billing/report closeout command-log guard requires sanitized dry-run/handoff/template/result command chain and rejects raw capture flags | this commit | review-packet Node tests, web guard, `ReleaseValidationRunbookContractTest`, doc/config/runtime/sensitive evidence guards, `git diff --check` passed | Done | D follow-up live Trial prerequisites or retry/idempotency/server-restart recovery evidence guard |
| 20260511T111751Z | D | D-05 guard | 5 / Phase 2 `OrcaClient` unique communication boundary: production server JDK HTTP usage is restricted to `OrcaTransport` / `OrcaHttpClient` plus push/master-update exceptions | this commit | `check-orca-transport-boundary.sh`, `ReleaseValidationRunbookContractTest`, web guard, doc/config/runtime/sensitive evidence guards, `git diff --check` passed | Done | D follow-up retry/idempotency/server-restart recovery evidence guard or live Trial prerequisites |
| 20260511T115706Z | D | D-06 guard | 18/19 retry/idempotency/UNKNOWN/double-send/restart recovery release guard; live Trial parent items remain open | this commit | `check-orca-retry-recovery-contract.sh`, focused Maven runbook contract/retry-recovery tests, doc/config/runtime/sensitive evidence guards, web guard passed | Done | Execute live retry/recovery only after same-RUN_ID runtime/preflight/manual approval/secret prerequisites |
