...
| E | `codex/orca-ehr-r2-worker-e-safety-ui` | R2-E01 medical safety UI, DADS/a11y, patient header/modal rollout | Active | 20260511T133244Z | this commit | - |
...
| 20260511T130908Z | E | E-03/DADS follow-on | 11/20 Phase 4 critical modal action priority and 44px target | this commit | targeted `CriticalOperationConfirmDialog.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue critical-operation confirmation coverage for chart finalize/amend/cancel or prescription flows |
| 20260511T124452Z | E | E-03 follow-on | 11/20 Phase 4 Charts finish critical confirmation modal | this commit | targeted `chartsActionBar.test.tsx` / `chartsPageDirtyDot.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue critical-operation confirmation coverage for chart finalize/amend/cancel or prescription flows |
| 20260511T122351Z | E | E-03 follow-on | 11/20 Phase 4 Diagnosis ORCA disease critical confirmation modal | this commit | targeted `CriticalOperationConfirmDialog.test.tsx` / `DiagnosisEditPanel*.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue critical-operation confirmation coverage for chart finalize/prescription/finish flows |
| 20260511T115550Z | E | E-02 follow-on | 11/20 Phase 4 Patients detail common patient header context | this commit | targeted `PatientsPage.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue remaining patient header gaps or extend critical-operation confirmation coverage |
| 20260511T111745Z | E | E-02 follow-on | 11/20 Phase 4 Reception accept workflow common patient header | this commit | targeted `ReceptionPage.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue patient header rollout for remaining Reception/Patients gaps or extend critical-operation confirmation |
| 20260511T101147Z | E | E-02 follow-on | 11/20 Phase 4 Mobile Images common patient header context | this commit | targeted `MobileImagesUploadPage.deeplink.test.tsx`, explicit `verify:web-guard`, `typecheck`, and `git diff --check` passed | Done | Continue patient header rollout for Reception/Patients gaps or extend next critical-operation confirmation |
