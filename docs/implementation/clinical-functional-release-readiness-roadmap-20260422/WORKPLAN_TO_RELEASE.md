...
15. RWO-06I: investigate and verify `surgeryOrder` / `手術` class `500` through a separate payload identity; v1 was safely tested but Trial-rejected and must not be repeated. RUN_ID `20260425T010143Z` prepared source-backed v2 candidate `150003110` with passing no-live wrapper/contract evidence. Run sanitized runtime readiness and at most one live Trial checkpoint only if prerequisites are available; otherwise continue independent no-live work.
