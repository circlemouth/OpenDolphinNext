...
| Non-S3 runtime profile weakens storage safety. | High | RUN_ID `20260423T060115Z` implemented `attachment.storage.mode=disabled` for the WebORCA Trial dev/runtime profile, rejects S3/MinIO/PHR S3 config mixing, keeps production-like startup S3-only, and makes storage-dependent routes/readiness fail closed. RUN_ID `20260423T080150Z` rechecked the Trial-only `medicalmodv2` handoff without introducing S3/MinIO/object-storage values. | Live Trial medicalmodv2 has not been run through the new profile yet because local non-S3 runtime inputs are missing; storage readiness remains a non-claim. | Run exactly one approved Phase4 `medicalmodv2` action through the safe wrapper after confirming the non-S3 runtime path is available. |
