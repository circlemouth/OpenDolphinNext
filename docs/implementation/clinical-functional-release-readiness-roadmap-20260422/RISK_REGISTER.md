...
| Browser runtime behavior only partially verified for Clinical Wave 1. | High | Artifact-free browser local persistence evidence exists for selected RWO-02 through RWO-05 workflows. | Full UI click-through browser coverage and fullflow are still missing. | Continue safe no-artifacts browser specs; do not use artifact-capturing fixtures. |
