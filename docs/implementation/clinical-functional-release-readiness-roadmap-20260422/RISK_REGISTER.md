...
| Browser runtime behavior only partially verified for Clinical Wave 1. | High | Artifact-free browser chart-open and local persistence evidence exists for selected RWO-02 through RWO-05 workflows. | Prescription/order/SOAP/disease full UI click-through browser coverage and fullflow are still missing. | Continue safe no-artifacts browser specs; do not use artifact-capturing fixtures. |
