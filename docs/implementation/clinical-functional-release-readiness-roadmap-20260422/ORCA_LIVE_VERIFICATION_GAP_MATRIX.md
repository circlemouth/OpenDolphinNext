...
| diseasev3 | Disease local readback evidence only; RUN_ID `20260424T030710Z` confirms current CRUD path is `/api/local/diagnoses` and disease master read is candidate lookup only. | No live disease CRUD claim. | no | no | local docs only | Owner-approved safe `diseasev3` wrapper/parser/success criteria and business scope before any live mutation. | yes | yes | yes |
| subjectivesv2 | SOAP local save explicitly does not call ORCA subjectivesv2; RUN_ID `20260424T030710Z` confirms current SOAP path is `/api/local/charts/subjectives`. | No live SOAP ORCA claim. | no | no | local docs only | Owner-approved safe `subjectivesv2` wrapper/parser/success criteria and business scope before any live mutation. | yes | yes | yes |
