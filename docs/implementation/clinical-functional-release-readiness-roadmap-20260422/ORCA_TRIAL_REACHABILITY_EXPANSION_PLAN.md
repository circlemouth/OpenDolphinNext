...
- RUN_ID `20260425T030245Z` added source-backed v2 payload `medicalmodv2_instruction_charge_trial_reachability_v2.json`, SHA-256 `043c2a657746820a96950d6c05e2179d65040123d677a028e9ab86bc9af98858`, using code `113001810`. The safe wrapper dry-run and focused contract test passed with no live ORCA execution. Keep live blocked until Trial disease/facility/monthly-billing preconditions are established.
...
- RUN_ID `20260425T030245Z` added source-backed v2 payload `medicalmodv2_base_charge_trial_reachability_v2.json`, SHA-256 `4c092e032dd6f56eb5542ad65b2b6b28a8e1c1c802900f83e795dbbdba7a403a`, using code `111000110`. The safe wrapper dry-run and focused contract test passed with no live ORCA execution. Live progression requires Trial encounter-state preconditions that permit first-visit billing.
...
- RUN_ID `20260425T030245Z` added source-backed v2 payload `medicalmodv2_injection_trial_reachability_v2.json`, SHA-256 `1af0b23246e8f9ee79879b28a09888ecc719ec8f6381e2b798cd63fa020e3300`, using procedure code `130000510` plus paired drug/material/comment rows. The safe wrapper dry-run and focused contract test passed with no live ORCA execution.
- RUN_ID `20260426T112213Z` added a focused no-live injection v2 contract preflight for the same payload identity. It verifies the explicit row-role order `procedure`, `main`, `material`, `comment`, code-shape classes, Request_Number `01`, classCode `01`, and Request_Number `02` / `03` / `04` exclusion, then reruns the safe wrapper dry-run with sanitized evidence only. No Trial live request or runtime master lookup was executed; live progression still requires runtime readiness, duplicate-live checkpoint preflight, and endpoint-specific sanitized business-success criteria.
...
- RUN_ID `20260425T020245Z` executed that v2 identity once through the sanitized wrapper. Runtime readiness was HTTP `200` / `200`; ORCA Trial returned HTTP `200`, parsed API result `80`, and endpoint-specific classification `businessRejected`. Do not repeat the unchanged v2 identity without no-live investigation plus a concrete fix or changed Trial/business precondition.
...
- RUN_ID `20260426T233244Z` added changed v3 payload `medicalmodv2_radiology_trial_reachability_v3.json`, SHA-256 `144850285178276d543ebb424610cbf91a2e188b8dc597f957cc882577c4a16a`, with body-part `002000099`, imaging fee `170027910`, and body-part comment `820181000`. RUN_ID `20260427T000309Z` registered that v3 identity in the phase4 manifest and reran the safe wrapper dry-run and focused contract tests. This remains no-live evidence only; live remains stopped until runtime readiness, duplicate-live checkpoint decision, endpoint-specific success criteria, stop conditions, and a sanitized preflight packet are recorded.
