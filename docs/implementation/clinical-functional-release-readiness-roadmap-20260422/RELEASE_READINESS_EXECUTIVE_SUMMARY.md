...
3. `指導料`, base-charge acceptance, injection, surgery, tests, radiology, SOAP, and disease CRUD Trial reachability remains missing. RUN_ID `20260424T031608Z` accepted endpoint-specific prescription and representative treatment/generic `medicalmodv2` L3 Trial evidence, but it is not fullflow or broad order matrix coverage. RUN_ID `20260424T044007Z` now lists documents/fees/order classes explicitly, RUN_ID `20260424T044803Z` shows `指導料` class `130` v1 is safely tested but Trial-rejected, and RUN_ID `20260424T050223Z` blocks `指導料` v2 pending a business/Trial data decision and records `baseChargeOrder/110` v1 as live `businessRejected`.
