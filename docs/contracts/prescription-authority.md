...
- latest candidate 取得時は保存済み candidate の `prescription_order_id` / `prescription_order_revision_id` / `prescriptionContentHash` と、同じ facility / chart revision の現在処方 order id / status / revision id / `content_hash` を server-side で照合する。不一致、現在 order/revision/hash 欠落、または現在 status が `FINAL` / `CHANGED` / `REISSUED` 以外の場合は `prescription_candidate_source_stale` を付け、`NEEDS_REVIEW` / `sendable=false` として返す。
...
- latest candidate response は保存済みの sanitized `candidate_json` と issue summary から再構成し、raw ORCA body、credential、保険詳細、voucher / sequential、client 提供 authority field を返さない。
- Web client の候補確認 surface は patient / acceptance / department / physician / insurance combination を確認表示にだけ使い、candidate response の処方 content hash 要約、RP / 診療区分 / 用法 / 薬剤行を送信前確認材料として表示する。candidate prepare / latest request は `chartRevisionId` だけを route path として送り、facility / patient / insurance / voucher / sequential / URL / digest を body や query として送らない。
...
- 処方変更後、同一 hash の再発行後、同じ chart revision に新しい処方 order が作られた後、または現在処方が中止/取消になった後に古い latest candidate を取得し、現在処方 source と一致している候補として送信前確認へ進める。
