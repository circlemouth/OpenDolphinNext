...
- latest candidate 取得時は保存済み candidate の `prescriptionContentHash` と現在の処方 revision `content_hash` を server-side で照合する。不一致または現在 hash 欠落時は `prescription_candidate_source_stale` を付け、`NEEDS_REVIEW` / `sendable=false` として返す。
...
- 処方変更後に古い latest candidate を取得し、現在処方と一致している候補として送信前確認へ進める。
