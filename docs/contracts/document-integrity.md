...
- `GET /api/charts/{chartId}/revisions/export` は chart revision JSON export の最小 contract とする。施設 ID は server-side session から解決し、request body / query の owner/facility は採用しない。応答は `exportSchemaVersion`, `exportHashAlgorithm`, `exportHash`, `revisionCount`, `eventCount`, revision list, event list を返す。summary JSON は allowlist された scalar key のみを投影する。reason/title/context 文字列に Authorization / Cookie / raw XML / SOAP body が混入している場合は redaction して返す。`exportHash` は redaction / allowlist 適用後の canonical export payload から計算し、allowlist 外 key や JSON key order では変化させない。`revisionCount` / `eventCount` は canonical hash material に含め、履歴行欠落の検出を補強する。`chart_document.current_revision_id` がある場合、対応する revision が export revision list に含まれない不整合は fail-closed で拒否する。
