...
- `POST /api/charts/document-drafts` は本番 public の診療録下書き作成 route とする。legacy `POST|PUT|DELETE /api/karte/document` と `PUT /api/karte/document/{id}` は public runtime に登録しない。facility と患者は server-side に解決し、client 提供の owner / facility / role / uri / digest / objectKey は採用しない。詳細は [chart-authority-api.md](chart-authority-api.md) を正本とする。
...
- `snapshot_manifest_json` は chart finalize API が server-side に生成する。v2 manifest は ORCA患者、受付、保険、病名、処方指示、medical candidate、ORCA operation / transmission / reconciliation の reference / hash / status metadata を固定し、`patientSnapshotStatus=IDENTIFIER_ONLY` や `PENDING_WORKER_INTEGRATION` を本番 manifest として許容しない。患者氏名、住所、電話番号、保険詳細、raw ORCA body、credential、Cookie、Authorization、CSRF token は export allowlist に出さない。
