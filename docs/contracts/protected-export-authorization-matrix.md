...
| `POST /api/charts/document-drafts` | Chart revision draft create | chart revision create | Facility and patient are resolved server-side from authenticated session and `karte_id`; client-provided owner/facility metadata is rejected. | Creates `chart_document` and `chart_revision(status=DRAFT)` only. | Legacy `POST|PUT|DELETE /api/karte/document` and title-only update routes are not public runtime routes. |
| `POST /api/charts/{*}/revisions/{*}/finalize` | Chart revision finalize | chart finalize | Facility is resolved from authenticated session and chart document. | Server calculates content hash and appends FINALIZED event. | Client-provided digest/role/owner is not authoritative. |
| `POST /api/charts/{*}/revisions/{*}/amend` | Chart revision amendment | chart revision event | Facility is resolved from authenticated session and chart document. | Creates a new revision and append-only event; original locked revision is not overwritten. | Reason text and raw sensitive payloads are not written to audit metadata. |
| `POST /api/charts/{*}/revisions/{*}/addendum` | Chart revision addendum | chart revision event | Same as amend. | Creates a new revision and append-only event; original locked revision is not overwritten. | Same as amend. |
| `POST /api/charts/{*}/revisions/{*}/cancel` | Chart revision cancellation | chart revision event | Same as amend. | Appends cancellation event without deleting locked body/SOAP/attachments. | UNKNOWN / ORCA outcome is not converted into chart finalization. |
