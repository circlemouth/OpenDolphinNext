...
`/api/local/encounters/{encounterKey}/close-and-send-to-billing` は通常外来の初回 ORCA 会計送信用 local workflow です。facility は認証済み request context と `encounter_projection` だけを authority とし、client 提供の patient / facility / insurance / voucher / sequential / `Medical_Uid` / classCode / URL / raw XML を受け取りません。`encounter_projection` に server-derived ORCA受付 ID と受付日時がない場合は `orca_acceptance_missing` で fail closed にし、患者・カルテ・ORCA transport lookup へ進みません。
