...
- `encounter_orca_acceptance_link` は `encounter_projection` と `orca_acceptance_cache` の read model link として扱う。`encounter_key`、server-derived ORCA受付ID、受付日、診療科、担当医、保険組合せを保持し、cache の `CANCELLED` / `DIFF_DETECTED` / `NEEDS_REVIEW` は `warning_status` と `changed_fields_json` にだけ同期する。link 更新は `encounter_projection.business_state` を直接変更しない。workflow state の削除・取消・会計状態変更は別の明示 server-side workflow で行う。
- `encounter_orca_acceptance_link` は `raw_sensitive_fields_excluded=true`、`client_provided_identifiers_trusted=false`、`server_derived_authority_required=true` を DB 制約で固定し、client-provided acceptance / department / physician / insurance を authority にしない。raw ORCA body、患者詳細、保険詳細、credential、Cookie、Authorization、CSRF は保持しない。
