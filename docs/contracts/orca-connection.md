...
- mirror response は ORCA projection を `diseases`、既存 local-only disease を `pendingLocalDiseases` の `layer=candidate` / `candidateKind=draftCandidate` / `sourceOfTruth=local-candidate` として分離する。ORCA unavailable 時に local-only disease を `diseases` へ fallback しない。ORCA projection は表示名だけでなく `Disease_Single` component 列、`Disease_Supplement_Single`、転帰受信 code、server 計算 `orcaSnapshotHash` を保持する。
...
- `encounter_orca_acceptance_link` は `encounter_projection` と `orca_acceptance_cache` の read model link として扱う。`encounter_key`、server-derived ORCA受付ID、受付日、診療科、担当医、保険組合せを保持し、cache の `CANCELLED` / `DIFF_DETECTED` / `NEEDS_REVIEW` は `warning_status` と `changed_fields_json` にだけ同期する。患者 cache 更新時は `patient_cache_status` / `patient_business_status` / `patient_warning_status` / freshness timestamp を同期し、保険 cache 更新時は `insurance_cache_status` / `insurance_warning_status` / freshness timestamp / field-level `insurance_changed_fields_json` を同期する。患者・保険 warning は stale/unresolved を default とし、server-side ORCA cache が current / found / unchanged と確認できた場合だけ `CLEAR` にする。
- `encounter_orca_acceptance_link` 更新は `encounter_projection.business_state` を直接変更しない。workflow state の削除・取消・会計状態変更は別の明示 server-side workflow で行う。`raw_sensitive_fields_excluded=true`、`client_provided_identifiers_trusted=false`、`server_derived_authority_required=true` を DB 制約で固定し、client-provided acceptance / department / physician / insurance / patient cache freshness を authority にしない。raw ORCA body、患者詳細、保険詳細、credential、Cookie、Authorization、CSRF は保持しない。
- Charts の `/api/local/encounters/{encounterKey}/medical-summary` は、同じ facility の `encounter_projection` と `encounter_orca_acceptance_link` だけを読んで payload `orcaContext` を返す。`orcaContext` は受付 ID/日付/診療科/担当医/保険組合せ、受付 warning、患者 cache status/business/warning/freshness、保険 cache status/warning/freshness、field-level changed fields に限定し、患者詳細・保険詳細・raw ORCA body・credential・storage key/digest は含めない。他施設 encounterKey、link 不整合、projection 取得不能では warning を client 値から合成せず fail closed または context 非表示にする。
...
- 共通台帳の状態名、UNKNOWN分類、reconciliation status、UI向けDTO境界は [orca-ledger-and-unknown-state.md](orca-ledger-and-unknown-state.md) を正本とする。`orca_operation.central_audit_trace_id` により central audit と相互参照し、ORCA URL、Basic認証、証明書情報、raw patient detail、raw ORCA body は台帳・監査・bundle に保存しない。
...
- Public `income-info` / report resource は transport response を返す前に server-side cache/snapshot command を作成して保存し、`orca_billing_cache` / `orca_report_snapshot` 保存失敗時は ORCA 応答自体が業務成功でも HTTP 503 として fail closed にする。これにより会計・帳票表示は保存済みの sanitized cache/snapshot と監査可能な hash 境界なしに成立しない。
...
- 帳票 binary upload は server-internal `OrcaReportBinaryStorageService` だけが行う。upload 前に DB snapshot の facility、server-generated object key、server-generated digest、`CURRENT` status、upload state を照合し、content SHA-256 が `server_storage_digest` と一致する場合だけ object storage に put する。digest mismatch / snapshot mismatch / storage disabled は upload 前に fail closed し、client から object key、digest、bucket、retention を受け取る public route は作らない。
- `/api/orca/official/reports/{type}` は snapshot 保存後、server-side storage が有効な runtime だけ `OrcaReportBinaryStorageService` へ内部 upload command を渡す。upload command は snapshot receipt の id/facility/server-generated key/digest と transport response body から作り、public request body から storage key/digest/retention を受け取らない。response に出せる storage 情報は `storageUploadStatus` と `reportBinaryAvailable` のみで、storage URI、object key、digest、bucket、raw Data_Id、帳票本文は返さない。storage 有効時の upload 失敗は HTTP 503 として fail closed にする。
...
- `qa-weborca-candidate-discovery.mjs` も exact preflight と同じ read-only mutation blocker を使う。patientmodv2、appointments/visits mutation、acceptance operation、medicalmodv2、diseasev3、local billing send、temporary medical reconcile の request は browser route で abort し、summary には sanitized `mutationPolicy` の route template、method、固定 reason だけを残す。candidate discovery の blocked mutation は proposal / Phase 3 handoff blocker であり、患者 ID、query、request body、保険 detail、credential、Cookie、Authorization、CSRF は evidence に保持しない。
- `qa-weborca-readonly-preflight.mjs` は patientgetv2、保険組合せ、appointment / acceptance readiness の read-only 証跡に限定する。preflight 中の patientmodv2、appointments/visits mutation、acceptance operation、medicalmodv2、diseasev3、local billing send、temporary medical reconcile は browser route で abort する。これらの request が記録された場合、summary は `mutationPolicy.blockedRequestCount>0` と `readonly_mutation_attempt_blocked` を返し、Phase 3 / billing-report live handoff の入力にしてはいけない。blocked request summary は route template、method、固定 reason のみを残し、query、request body、患者 ID、保険 detail、credential、Cookie、Authorization、CSRF は保持しない。
- Phase 3 / billing-report live handoff の preflight artifact は `qa/weborca-readonly-preflight/summary.json` の exact selected-candidate summary path に限定する。`QA_READONLY_PREFLIGHT_SUMMARY` が candidate discovery、acceptmodv2、fullflow、network、HAR/trace/video/screenshot artifact を指す場合は、summary 内容や hash が揃っていても handoff 前に fail closed とする。
...
- operator が live Trial を実行した場合の記録は `qa-orca-billing-report-live-result.mjs` で sanitized result に正規化する。accepted evidence は `orca_billing_cache` / `orca_report_snapshot` の hash、件数、invoice/data id hash、server-generated storage key/digest presence、`storageUploadStatus`、`reportBinaryAvailable` だけで、raw ORCA body、帳票本文、raw patient / invoice / `Data_Id` / `Medical_Uid`、storage URI/key/digest、credential、browser diagnostic artifact は拒否する。
...
- `encounter_projection` に server-derived ORCA受付 ID と受付日時がない場合、通常外来の初回 ORCA 会計送信は `orca_acceptance_missing` で fail closed にする。client が voucher / sequential / insurance / patient を補っても authority にせず、患者・カルテ・ORCA transport lookup の前に止める。`worklist_flags_json.officialVisitIdentifiers.voucherNumber` は `encounter_projection.orca_acceptance_id` と一致しなければならず、不一致時は同じく ORCA transport 前に fail closed にする。
- 既存の同一 idempotency transmission は保存済み結果として返すが、新規 idempotency の通常初回送信は `accounting-wait`、`billing-waiting`、`billed`、`closed`、`chart-finalized`、`orca-sent`、`orca_medical_registered` など会計送信済み/閉鎖相当の `encounter_projection.business_state` では `encounter_billing_send_blocked` として fail closed にし、患者・カルテ・ORCA transport lookup へ進めない。
...
- `POST /api/local/encounters/orca-transmissions/{transmissionId}/reconcile-temporary-medical` は `ORCA_UNKNOWN`、`ORCA_FAILED`、`CORRECTION_REQUIRED` の review transmission だけを対象に、認証 principal の facility と保存済み snapshot から ORCA `tmedicalgetv2` read-only 照合を行う。request body は受け付けず、body が存在する場合は audit/repository/ORCA lookup 前に fail closed にする。patient / facility / insurance / voucher / sequential / `Medical_Uid` / URL / raw XML はすべて server 側の保存済み record/snapshot からだけ解決する。保存済み snapshot は `rawSensitiveFieldsExcluded=true`、`clientProvidedIdentifiersTrusted=false`、`serverDerivedAuthorityRequired=true` を満たす必要があり、未サニタイズまたは client-trusted snapshot は ORCA transport 前に `SERVER_CONTEXT_INCOMPLETE` とする。照合 request は保存済み snapshot の診療日と診療科、保存済み record の患者番号から作り、snapshot の診療日または診療科が欠落している場合も ORCA transport 前に `SERVER_CONTEXT_INCOMPLETE` とする。ORCA response が parse 不能または `Api_Result` 欠落の場合は `apiResult=unknown` と sanitized message へ正規化し、raw body や parser 詳細を返さず `needsUserReview=true` にする。ORCA `Api_Result` が `"00"` でない場合は response row を一致候補にせず、`Medical_Uid` presence や `resendBlocked` 判定に使わない。ORCA response row に診療日が含まれる場合は保存済み snapshot の診療日と一致する row だけを一致候補にする。response には一致件数、`Medical_Uid` の存在有無、`Medical_Mode` / `Medical_Mode2`、固定 `needsUserReview=true`、必要時の `resendBlocked` / `resendBlockReason` だけを返す。`Medical_Uid` 値、保険組合せ、raw ORCA body、資格情報、患者氏名、住所、電話番号は response / audit / tracked evidence に残さない。照合成功は再送成功や会計反映済みを意味せず、state transition は別の明示操作で行う。`Medical_Mode` または `Medical_Mode2` が空でなく `0` 以外の場合は fail-closed に `resendBlocked=true`、`resendBlockReason=ORCA_TEMPORARY_MEDICAL_MODE_LOCKED` とし、原則再送を止めて管理者確認に回す。
