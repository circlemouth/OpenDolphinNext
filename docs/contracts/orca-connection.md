...
- `qa-weborca-readonly-preflight.mjs` は patientgetv2、保険組合せ、appointment / acceptance readiness の read-only 証跡に限定する。preflight 中に patientmodv2、appointments/visits mutation、acceptance operation、medicalmodv2、diseasev3、local billing send、temporary medical reconcile の request が記録された場合、summary は `mutationPolicy.blockedRequestCount>0` と `readonly_mutation_attempt_blocked` を返し、Phase 3 / billing-report live handoff の入力にしてはいけない。blocked request summary は route template、method、固定 reason のみを残し、query、request body、患者 ID、保険 detail、credential、Cookie、Authorization、CSRF は保持しない。
