...
- [ ] `algorithm` は `HMAC-SHA256` 固定。
- [ ] `status=active` はちょうど 1 件。
- [ ] `keyId` は重複不可。
- [ ] Base64 デコード後 32 bytes 以上。
- [ ] `verify-only` キーは verify に使えるが seal には使わない。
...
## 実装タスク
- [ ] `DocumentIntegrityConfig` を keyring loader へ書き換える。
- [ ] `DocumentIntegrityService.verifyDocumentOnRead()` から active key との一致判定を削除する。
- [ ] `ServerConfigurationValidator` に keyring validation を追加する。
- [ ] 監査 payload から raw secret と不要な差分情報を除く。
- [ ] key rotation / missing key / permissive / enforce のテストを追加する。

## 受け入れ条件
- [ ] active key を変更しても旧文書が verify できる。
- [ ] `mode=enforce` で only 409 を返し、`mode=permissive` は読み取り継続する。
- [ ] malformed keyring / active key 複数 / keyId 重複で起動失敗する。
