...
## 実装タスク
- [ ] `LogFilter` の匿名許可対象から詳細 readiness を外す、または `/api/operations/readiness` を新設して認証必須にする。
- [ ] `OperationsHealthResource` を liveness / minimal readiness / authenticated readiness の 3 契約に分離する。
- [ ] `RestOrcaTransport.ProbeResult` から URL / statusCode / raw message 依存を外し、sanitized reasonCode を返す。
- [ ] `AttachmentStorageManager` または専用 health probe に backend 疎通 API を追加する。
- [ ] `PvtService.workerHealthBody()` の reason を fixed reasonCode に正規化する。
- [ ] JSON 契約テストを追加する。

## 受け入れ条件
- [ ] 匿名 readiness 応答に `checks` フィールドが存在しない。
- [ ] 認証付き readiness でも URL / host / port / stack trace が JSON に含まれない。
- [ ] ORCA / storage / PVT / patient images の DOWN ケースを固定 reasonCode で返す。
- [ ] ログと API 応答の双方で secret と接続先詳細を出力しない。
