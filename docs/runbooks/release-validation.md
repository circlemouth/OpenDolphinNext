...
- `qa-weborca-candidate-discovery.mjs` は `QA_CANDIDATE_DISCOVERY_TIMEOUT_MS` で上限を持つ。既定は `180000`ms。timeout 時は `summary.json` / `summary.md` / exact preflight placeholder に `blockerClassification=candidate_discovery_timeout`、`acceptedForPhase3Attempt=false`、`liveMutationExecuted=false`、`rawSensitiveFieldsExcluded=true` を書いて exit `124` で停止し、raw network / screenshot / HAR / trace / video を retained evidence にしない。timeout summary は exact selected-candidate preflight の代替ではない。
