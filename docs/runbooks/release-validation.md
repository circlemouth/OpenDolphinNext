...
- discovery で `acceptedForPhase3Attempt` の最初の 1 件が見つかった場合でも、その summary は sanitized selected-candidate proposal です。Phase 3 handoff artifact には使わず、同じ RUN_ID / selected candidate で `qa-weborca-readonly-preflight.mjs` を実行する。accepted candidate が 0 件の場合も、公式初期患者 `00001`〜`00011` が存在しないとは結論しない。verdict は `PARTIAL / TEST-DATA OR HARNESS READINESS BLOCKER` とし、意味は current harness/API/auth/ID normalization/response parser/insurance readiness/appointment dependency/exact preflight criteria によって mutation-ready read-only evidence が未充足であることに限定する。
