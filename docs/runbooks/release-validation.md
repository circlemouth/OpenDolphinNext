...
- `runtime-ready-smoke` は current query date の official visit row が見つからない場合も raw artifact を残さず、`runtime-ready-result.json` に `verdict=blocked`、`blockerClassification=runtime_ready_not_ready`、`liveMutationExecuted=false`、`rawSensitiveFieldsExcluded=true` を書いて非 0 終了する。この blocker evidence は mutation / fullflow / billing-report 実行許可ではない。
...
- `qa-weborca-candidate-discovery.mjs` は read-only 限定で、official patient existence、insurance readiness、selector readiness、local selectable、appointment dependency、diagnostic no patient-not-found を candidate ごとに評価する。candidate row には `insuranceReadiness.status/apiResult/classification/accepted` と `appointmentDependency.flowMode/required/status/apiResult/classification/accepted` を残す。official 初期患者情報は category / count 程度に sanitization し、氏名・住所・電話番号・保険詳細など raw sensitive detail は summary / row に残さない。mutation 系 route が発火した場合は script 側で abort し、candidate を accepted にしない。
- `qa-weborca-candidate-discovery.mjs` は `QA_CANDIDATE_DISCOVERY_TIMEOUT_MS` で全体上限を持つ。既定は `180000`ms。各候補評価は `QA_CANDIDATE_EVALUATION_TIMEOUT_MS` で個別上限を持つ。既定は `30000`ms。候補 row は 1 件ごとに `candidate-rows.json` へ逐次保存し、server-side official / insurance / appointment readiness が rejected の候補では UI selector probe を省略して rejected row にする。timeout 時は `summary.json` / `summary.md` / exact preflight placeholder に `blockerClassification=candidate_discovery_timeout`、`partialCandidateCount`、`acceptedForPhase3Attempt=false`、`liveMutationExecuted=false`、`rawSensitiveFieldsExcluded=true` を書いて exit `124` で停止し、`QA_SANITIZED_EVIDENCE_ONLY=1` では raw network / screenshot / HAR / trace / video / `network/*.json` を retained evidence にしない。timeout summary と partial selected proposal は exact selected-candidate preflight の代替ではない。
...
- `qa-acceptmodv2-weborca.mjs` は既定で同じ `RUN_ID` の exact selected-candidate `qa/weborca-readonly-preflight/summary.json` を要求する。preflight artifact を別パスに置いた場合は `QA_READONLY_PREFLIGHT_SUMMARY` を渡し、preflight accepted でない run や discovery proposal しかない run では mutation を実行しない。
...
- fullflow では send 到達の有無にかかわらず `request-xml/medicalmodv2.xml`、HAR、raw network dump、raw request/response body を reviewer packet 正本へ含めない。`QA_SANITIZED_EVIDENCE_ONLY=1` / `QA_DISABLE_BROWSER_ARTIFACTS=1` の fullflow harness は `network/`、`screenshots/`、`har/`、`request-xml/` を retained evidence として作らず、`summary.json`、`blocker-summary.json`、`handoff-state.json`、`selected-visit-row.json` も raw patient row text や raw `Medical_Uid` 値を持たない allowlist summary に限定する。診断 harness が raw artifact を local-only / untracked に生成した場合でも、third party が再読する証跡は allowlisted completion/status fields、hash、diagnostic artifact manifest に限定する。
