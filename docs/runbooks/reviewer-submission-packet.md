...
- accept / fullflow / billing-report-live-profile / billing-report-live-result / report / probe summary が `raw-*.xml`、`server-stacktrace.log`、`har/`、`*.har`、`request-xml/medicalmodv2.xml`、`network/network.json`、`network/requests.json` を参照していたら packet validate は fail する。
