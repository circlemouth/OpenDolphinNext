...
- chart/PDF/export/attachment evidence は [protected-export-authorization-matrix.md](../contracts/protected-export-authorization-matrix.md) の matrix に従う。closeout report / QA / evidence が `chart-export-raw.*`、`raw-chart-export.*`、`chart-revision-raw.*`、`chart-revisions-export.*`、`period-export.*`、`printed-chart.*`、`report-body.*`、`report-binary.*`、`patient-image-download.*`、`attachment-download.*`、`storage-uri.*` のような raw protected export artifact を参照していたら packet validate は fail する。提出物には sanitized count/hash/metadata と matrix/doc reference だけを含める。
